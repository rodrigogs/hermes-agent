...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    try:
        ...
        # been the one requested. This write is fork-only — upstream's fallback
        # path (which now owns the buffer + pending-list machinery below) does
        # not touch the session row, so the 2026-08-26 merge keeps the call and
        # takes upstream's notice text, which additionally names the reason.
