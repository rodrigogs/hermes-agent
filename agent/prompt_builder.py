...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "mode: in standard mode ask the USER to run `hermes computer-use "
        "browser-approve` and pass the pasted single-use token as "
        "`approval_token` (you can never mint one); bounded mode authorizes "
        "via the user's reviewed capability manifest; explicit Hermes YOLO "
        "uses a private unrestricted daemon after the user's launch/session "
        "risk acceptance.\n\n"
