...
def _classify_by_status(
    ...
    *,
    ...
    if status_code == 429:
        ...
        #
        # 2026-08-26 merge: upstream generalised the same idea and its condition
        # SUBSUMES this fork's — ``has_billing`` below is exactly the test that
        # used to stand alone here — while adding two guards this fork lacked:
        # a body that is itself an explicit rate-limit phrase stays a rate limit
        # ("Rate limit exceeded" contains "limit exceeded"), and a periodic quota
        # that ships an explicit reset/retry signal stays transient. The z.ai
        # 1113 body carries neither, so the case the paragraph above was written
        # for still routes to billing.
        ...
        # As duas guardas do upstream aplicam-se ao ALARGAMENTO (usage-limit), nao
        # a uma frase explicita de cobranca — o proprio comentario acima diz
        # "regardless of usage-limit wording". Este fork mediu o caso em que a
        # diferenca aparece: o corpo 1113 da z.ai diz "insufficient balance" E
        # convida a tentar de novo, e o teste
        # test_429_with_insufficient_balance_is_billing_not_rate_limit reprova a
        # condicao do upstream literal (vira rate_limit, ganha cooldown de uma
        # hora, e o saldo continua zero por dias). Cobranca explicita vence as
        # duas guardas; o alargamento continua sujeito a elas.
        if (
            has_billing
            or (
                has_usage_limit
