...
DELEGATE_BLOCKED_TOOLS = frozenset(
    [
        ...
        # FORK DIVERGENCE (deliberate, security posture): upstream leaves
        # execute_code available to children so they can batch mechanical work
        # (Teknium, Jul 2026). This install denies programmatic code execution
        # in delegated subagents. Keeping it here — one frozenset entry — instead
        # of forking the gating logic is what makes it survive a sync: the
        # `code_execution` toolset is stripped automatically, because
        # _strip_blocked_tools derives toolset removal from this very set.
        "execute_code",
    ]
)
...
def _build_child_agent(
    ...
    #
    # EXCEPT when the user pinned delegation.provider (or a named profile
    # was selected): an explicit pin means "children run on THIS provider".
    # Inheriting the parent chain would let a mid-run auth/429 failure
    # silently reroute the quiet-mode child onto the parent's fallback
    # models with no surfaced signal (#80450) — the same class of
    # silent-drag the override_provider filter-clearing below already
    # prevents for OpenRouter routing preferences. A named profile also
    # disables it: falling back through the parent's chain would silently
    # abandon the selected profile's identity, credentials, billing and
    # audit trail. Predictability > liveness for explicit pins: the pinned
    # child fails loudly instead.
    parent_fallback = (
        None
        if (override_provider or profile_name)
...
def delegate_task(
    ...
    # LOCAL (not upstream): route children to a named delegation profile's
    # own provider/model/credentials and toolsets instead of inheriting the
    # parent's. A per-task "profile" key overrides this top-level default.
    ...
    for i, t in enumerate(task_list):
        ...
        # LOCAL: a selected profile supplies its own runtime and
        # toolsets. With no profile, `runtime` is the parent's creds,
        # so behaviour is exactly upstream's.
...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        "- Pass 'profile' (top-level or per task) to run a child with that "
        "named profile's SOUL persona, runtime credentials, model and toolsets "
        ...
        "properties": {
            ...
            "profile": {
                ...
                "description": (
                    ...
                    "Routes children to that profile's SOUL persona, runtime "
                    "credentials, model and toolsets."
                ),
            },
